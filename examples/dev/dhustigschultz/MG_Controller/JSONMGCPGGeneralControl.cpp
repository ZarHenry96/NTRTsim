...
/**
 * @file JSONMGCPGGeneralControl.cpp
...
JSONMGCPGGeneralControl::Config::Config(int ss,
										int tm,
										int om,
										int param,
										int segnum,
 ...
 */
JSONMGCPGGeneralControl::JSONMGCPGGeneralControl(JSONMGCPGGeneralControl::Config config,	
												std::string args,
...
JSONMGCPGGeneralControl::~JSONMGCPGGeneralControl() 
{
    scores.clear();
}

void JSONMGCPGGeneralControl::onSetup(BaseQuadModelLearning& subject)
{
    // Maximum number of sub-steps allowed by CPG
	m_pCPGSys = new CPGEquations(2000);
...
void JSONMGCPGGeneralControl::setupCPGs(BaseQuadModelLearning& subject, array_2D nodeActions, array_3D edgeActions)
{
...
#else
		tgCPGMGActuatorControl* pStringControl = new tgCPGMGActuatorControl();
    ...
    for (std::size_t i = 0; i < m_allControllers.size(); i++)
    {
        tgCPGMGActuatorControl * const pStringInfo = m_allControllers[i];
...
void JSONMGCPGGeneralControl::onStep(BaseQuadModelLearning& subject, double dt)
{
    m_updateTime += dt;
...
void JSONMGCPGGeneralControl::onTeardown(BaseQuadModelLearning& subject)
{
    scores.clear();
...
const double JSONMGCPGGeneralControl::getCPGValue(std::size_t i) const
{
	// Error handling on input done in CPG_Equations
	return (*m_pCPGSys)[i];
}

double JSONMGCPGGeneralControl::getScore() const
{
	if (scores.size() == 2)
	{
		return scores[0];
...
array_3D JSONMGCPGGeneralControl::scaleEdgeActions  
                            (Json::Value edgeParam)
{
    ...
    * note that j==k will refer to the same muscle
...
array_2D JSONMGCPGGeneralControl::scaleNodeActions  
                            (Json::Value actions)
{
