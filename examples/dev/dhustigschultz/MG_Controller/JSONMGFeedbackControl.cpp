...
/**
 * @file JSONMGFeedbackControl.cpp
...
JSONMGFeedbackControl::Config::Config(int ss,
                                        int tm,
                                        int om,
                                        int param,
                                        int segnum,
					...
					double maxH,
					double minH) :
JSONMGCPGGeneralControl::Config::Config(ss, tm, om, param, segnum, ct, la, ha,
 ...
 */
JSONMGFeedbackControl::JSONMGFeedbackControl(JSONMGFeedbackControl::Config config,	
                                                std::string args,
                                                std::string resourcePath) :
JSONMGCPGGeneralControl(config, args, resourcePath),
m_config(config)
{
...
JSONMGFeedbackControl::~JSONMGFeedbackControl()
{
    delete nn;
}

void JSONMGFeedbackControl::onSetup(BaseQuadModelLearning& subject)
{
    m_pCPGSys = new CPGEquationsFB(1000000);
    ...
    array_3D edgeParams = scaleEdgeActions(edgeVals);
...
void JSONMGFeedbackControl::onStep(BaseQuadModelLearning& subject, double dt)
{
    m_updateTime += dt;
    m_totalTime += dt;
...
void JSONMGFeedbackControl::onTeardown(BaseQuadModelLearning& subject)
{
    scores.clear();
    metrics.clear();
...
void JSONMGFeedbackControl::setupCPGs(BaseQuadModelLearning& subject, array_2D nodeActions, array_3D edgeActions)
{
	    
    std::vector <tgSpringCableActuator*> spineMuscles = subject.find<tgSpringCableActuator> ("all ");
    ...
    for (std::size_t i = 0; i < spineMuscles.size(); i++)
    {
        ...
        tgCPGMGCableControl* pStringControl = new tgCPGMGCableControl(config);
    ...
    {
        tgCPGMGActuatorControl * const pStringInfo = m_spineControllers[i];
...
array_2D JSONMGFeedbackControl::scaleNodeActions (Json::Value actions)
{
...
array_3D JSONMGFeedbackControl::scaleEdgeActions  
                            (Json::Value edgeParam)
{
    ...
    array_3D actionList(boost::extents[m_config.theirMuscles]
										[m_config.ourMuscles]
										[m_config.params]);
            ...
            while(k < m_config.ourMuscles)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        // Weight from 0 to 1
                        actionList[j][k][0] = edgeParam[0].asDouble();
                        //std::cout << actionList[j][k][0] << " ";
                        // Phase offset from -pi to pi
                        actionList[j][k][1] = edgeParam[1].asDouble() * 
                                                (range) + lowerLimit;
                        //std::cout <<  actionList[j][k][1] << std::endl;
                        count++;
                    }
                }
                k++;
...
std::vector<double> JSONMGFeedbackControl::getFeedback(BaseQuadModelLearning& subject)
{
    // Placeholder
    std::vector<double> feedback;
    
    const std::vector<tgSpringCableActuator*>& spineCables = subject.find<tgSpringCableActuator> ("all ");
...
std::vector<double> JSONMGFeedbackControl::getCableState(const tgSpringCableActuator& cable)
{
...
std::vector<double> JSONMGFeedbackControl::transformFeedbackActions(std::vector< std::vector<double> >& actions)
{
	// Placeholder
	std::vector<double> feedback;
