...
#ifndef BIGPUPPY_SPINEONLY_STATS_H
#define BIGPUPPY_SPINEONLY_STATS_H

/**
 * @file BigPuppySpineOnlyStats.h
...
class BigPuppySpineOnlyStats: public BaseQuadModelLearning //public tgSubject<BigPuppy>, public tgModel
{
public: 

    BigPuppySpineOnlyStats(int segments, int hips);

    virtual ~BigPuppySpineOnlyStats();
    ...
    //std::vector<double> getCOM(const int n) const;
...
private:
    ...
    */
    void addSegments(tgStructure& puppy, tgStructure& vertebra, tgStructure& hip, double r); //, std::size_t segments, std::size_t hips, std::size_t legs, std::size_t feet
