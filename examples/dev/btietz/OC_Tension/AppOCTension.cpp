...
AppOCTension::AppOCTension(int argc, char** argv)
{
    bSetup = false;
    use_graphics = false;
    add_controller = true;
...
bool AppOCTension::setup()
{
    ...
    startAngle = ((rand() / (double)RAND_MAX) - 0.5) * 3.1415;
      ...
      new OctahedralComplex(nSegments, goalAngle, startAngle);
    ...
    if (add_controller)
    {
        ...
        std::string resourcePath = "bmirletz/OC_Tension/";
        ...
        OctahedralTensionControl* const myControl =
        new OctahedralTensionControl(control_config, suffix, resourcePath);
...
void AppOCTension::handleOptions(int argc, char **argv)
{
...
const tgHillyGround::Config AppOCTension::getHillyConfig()
{
...
const tgBoxGround::Config AppOCTension::getBoxConfig()
{
...
tgModel* AppOCTension::getBlocks()
{
...
tgWorld* AppOCTension::createWorld()
{
...
tgSimViewGraphics *AppOCTension::createGraphicsView(tgWorld *world)
{
    return new tgSimViewGraphics(*world, timestep_physics, timestep_graphics);
}

tgSimView *AppOCTension::createView(tgWorld *world)
{
    return new tgSimView(*world, timestep_physics, timestep_graphics);
}

bool AppOCTension::run()
{
    if (!bSetup)
    {
        setup();
...
void AppOCTension::simulate(tgSimulation *simulation)
{
...
{
    std::cout << "AppOCTension" << std::endl;
    AppOCTension app (argc, argv);
