...
function init_config()
{
    ...
    if [ -f "$CONF_DIR/install.conf" ]; then
        echo "Your conf directory contains install.conf, which has been deprecated.\
             Please delete install.conf and run set up again. See issue 21 for more details."
        exit 1
    fi
...
run_setupscript "googletest" "GoogleTest"
...
echo ""
echo "Setup of external dependencies completed!"
echo ""
echo "Now is possible to build NTRT library with cmake."
echo ""
